...
In this article, we are diving into a 3D demo and discuss how to optimize its performance step by step with React 18's concurrent rendering. The final outcome looks like this:
...
React 18 introduces the concurrent rendering feature. The core idea is to classify state updates into two categories:
...
- **Skipping old results**: React will discard outdated transition rendering and only render with the newest state.

## Demo Overview

In order to stress test the concurrent rendering, I built a 3D demo with
...
- **Search Bar**: to search for a GitHub username and compute the coordinates of the user and her following users and followers on GitHub.
- **Scene**: to visualize the connections between the user and her connections in on a 3D globe.

> You can find [the repository on GitHub](https://github.com/DawChihLiou/six-degrees) and [the live demo on Netlify](https://sweaty-concurrent-react.netlify.app).

As we type into the search bar, you should see an increasing amount of stars in the scene to simulate the high rendering demand on the UI.
...
As I was typing, the search bar was frozen. It didn't display the input value until the very end. Not just the search bar, the scene was frozen too. The stars didn't get rendered until I finished typing. Ideally, we want to be able to observe immediate responses from our typing.
...
Because React will batch the state updates together in the same event for better performance, we need additional work to separate the renderings. Let's take a look at the typical approaches.
...
However, we noticed that the `Scene` is continuously rendered updated multipal times after we were done typing. That's because the renderings were still scheduled when the `setTimeout`s were fired.

### Approach #2: Debouncing

We can actually cancel the rendering schedule by using debouncing.
...
The user experience looks better now. The `SearchBar` remains fairly responsive with 4x slowdown and the redundant updates on the `Scene` after typing is gone.

However, the shortcoming of debouncing is that the `Scene` update will always be delayed by 100ms. With faster CPU, the delay is most likely redundant and it makes the page feel slow.
...
We call `setUsername` inside of the `startTransition` function. It tells React to defer `setUsername` to the other urgent updates.
...
- The keypress events are separated from the long rendering function calls. It has a similar pattern as with setTimeout.
...
The **interrupting** behavior is not so straight forward to observe but the fact that 5 rendering calls only resulted in 2 UI updates, I assume the transition updates did defer to the keypress events when they happens.
...
`SearchBar` is always responsive while `Scene` updates quickly. The user experience feels so much smoother.

## Final Thoughts

In the past, adding debouncing was my go-to solution for rendering large UI updates while keeping pages responsive. Based on this experiment, concurrent rendering makes the UI more responsive without the shortcoming of debouncing. Even for the heavy computation with at least 100,000 3D objects, concurrent rendering still helped the page to be snappy.
...
- [Demo repository](https://github.com/DawChihLiou/six-degrees)
- [Demo Site](https://sweaty-concurrent-react.netlify.app)
- [New feature: startTransition](https://github.com/reactwg/react-18/discussions/41)
- [Real world example: adding startTransition for slow renders](https://github.com/reactwg/react-18/discussions/65)
- [WebGL Globe Solution](https://webgl-globe-solutions.stackblitz.io)
- [three.js](https://github.com/mrdoob/three.js/)
- [react-three-fiber](https://github.com/pmndrs/react-three-fiber)
