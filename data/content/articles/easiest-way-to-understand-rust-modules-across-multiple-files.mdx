...
- 🤿 We'll deep dive into a real-word example to explore the module system.
...
```rust:src/a/c.rs
pub fn do_c {
    crate::a::do_b(); // absolute path
    super::do_b(); // relative path
}
```
...
The [**use**][2] declaration can help you shorten the path when accessing an item in another module. For example, we can refactor the module "a":
...
It creates a local name binding to its path for _do_b_ and _do_c_. **use** is very useful for long paths.
